...
import stretch from './assets/images/stretch.gif'
import wat from './assets/images/wat.gif'
import questions from './assets/images/questions.gif'
import sevenPotters from './assets/images/seven-potters.gif'
...
---
# How do I type the shiny new JavaScript pattern?
...
```ts file=./snippets/properties-never-unknown.ts
```
...
<img src={sevenPotters} style={{height: '90%'}} />

---

<CodeSurfer>

```ts file=./snippets/properties-any-never-unknown.ts
```
...
# Generics are like passing a type as an argument
---
...
# We can only pass one of these types
...
```ts 9[31:56],11 file=./snippets/indexing.ts
```
```ts 9[41:56] file=./snippets/indexing.ts
```
```ts file=./snippets/keyof-index.ts
```
...
```ts 7,8 file=./snippets/keyof-example.ts
```
...
<img src={wat} style={{height: '90%'}} />

---

<img src={stretch} style={{height: '90%'}} />
...
```ts 4:6 file=./snippets/conditional-syntax.ts
```
...
```
```ts 1,2 file=./snippets/infer-syntax.ts
```

```ts 1[24:50] file=./snippets/infer-syntax.ts
```

```ts 1[43:49] file=./snippets/infer-syntax.ts
```

```ts 2[5] file=./snippets/infer-syntax.ts
```

```ts 2[8:13] file=./snippets/infer-syntax.ts
```


```ts 4,5 file=./snippets/infer-syntax.ts
```
```ts 1,2,4,7 file=./snippets/infer-syntax.ts
```
```ts 1,2,5,8 file=./snippets/infer-syntax.ts
```
```ts 1,2,9 file=./snippets/infer-syntax.ts
```
...
```
```ts 1,2 file=./snippets/infer-function.ts
```
...
```ts 1,2 file=./snippets/infer-props.ts
```
...
# Use never to tell typescript something is broken
...
<img src={questions} style={{height: '90%'}} />
